...
from grpcs.client_wrapper import ServiceClient

import user_pb2_grpc as users_service
...
def users_save():
    ...
    request2 = users_messages.GetUsersRequest(
        user=[users_messages.User(username=content['user']['username'], user_id=content['user']['userId'])]
    )

    response = MessageToJson(app.config['users'].CreateUser(request2))
